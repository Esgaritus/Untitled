...
class Dragon (pg.sprite.Sprite):
	def __init__(self, x, y, a, b):
		...
		self.image=pg.Surface([32*a,32*b])
...
class Mapa1(object):
	def __init__(self, archivo):
		...
		for i in self.base['layers']:
			...
			if i['name'] == 'Espantapajaros':
				self.lsEspant = i['data']
		...
		self.lyEspant = self.Separar(self.lsEspant, self.AnchoF)

		self.lstiles = self.Tiles()

	def Mapeo(self, BaseC2, BaseC, BaseNC, Sombras1, Sombras2, Sombras3, ObjetosC, DetallesNC, Espant):

		nf = 0
		for f in self.lyEspant:
			ne = 0
			for e in f:
				if e != 0:
...
class Mapa2(object):
	def __init__(self, archivo):
		...
		for i in self.base['layers']:
			if i['name'] == 'Colisiona':
				self.lsColisiona = i['data']
			if i['name'] == 'Fondo':
				self.lsFondo = i['data']

		self.AnchoF = self.base['width']
		self.AltoF = self.base['height']
		self.lyColisiona = self.Separar(self.lsColisiona, self.AnchoF)
		self.lyFondo = self.Separar(self.lsFondo, self.AnchoF)

		self.lstiles = self.Tiles()

	def Mapeo(self, Colisiona, Fondo):
